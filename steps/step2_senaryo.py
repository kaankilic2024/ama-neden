...
SISTEM = """Sen yetiskinler icin merak videosu senaryosu yazan bir uzmansin.

KANAL: {kanal_adi}
TANIM: {kanal_tanimi}

GOREVIN: Verilen "neden" sorusunu, sahne sahne bir video senaryosuna cevirmek.

YAPI (cok onemli):
1. Soruyu sor -- izleyicinin merak etmesini sagla
2. Merakı derinlestir -- "aslinda dusundugunden daha ilginc" hissi ver
3. Cevabi adim adim ac -- tek cumlede verme, zincirleme anlat
4. Toparla -- izleyici "anladim" hissiyle ayrilsin

Cevabi HEMEN verme. Ilk sahnede soruyu sor, cevabi ortalarda ac.
Ama cevabi cok da geciktirme; izleyici sabirsizlanmasin.
...
- Uydurma sayi, uydurma isim, uydurma calisma KESINLIKLE YASAK.
- "ILK / EN / TEK" IDDIASI YASAK: "dunyanin ilk", "en buyuk", "tek ornek",
  "kesinlikle", "kanitlanmistir" gibi ifadeler kullanma.
- Kesin sayi yerine yaklasik ifade: "tam 3.472" degil "binlerce".
- Tartismali konularda "arastirmacilarin cogu", "yaygin gorus" gibi
  temkinli ifadeler kullan.
...
- Somut ol: soyut kavram yerine onu temsil eden somut bir sahne tarif et.
  YANLIS: "the concept of gravity"
  DOGRU : "an apple falling from a tree branch, motion blur, close-up"
- STIL KELIMESI YAZMA ("illustration", "cinematic" vb.). Sistem ekleyecek.
- Gercek kisilerin yuzunu tarif etme.
- Goruntude yazi, harf, rakam olmasin.
- CESITLILIK: Ardisik sahneler benzemesin; cekim olcegini degistir
  (wide shot / medium shot / close-up / overhead / macro) ve mekani kaydir.
- Prompt 20-35 kelime olsun.

Cevabini SADECE su JSON formatinda ver, baska hicbir sey yazma:
{{
  "baslik": "YouTube basligi, soru formatinda, 70 karakteri gecmesin",
  ...
  "sahneler": [
    {{
      ...
      "gorsel_prompt": "English: concrete scene, setting, light, composition"
    }}
  ]
}}"""
...
MOCK_SENARYO = {
    ...
    "aciklama": "Havaalanında etrafına bak: uçakların neredeyse hepsi beyaz. "
                ...
                "#amaneden #merak #havacılık #ilginçbilgiler #neden",
    "etiketler": ["ama neden", "uçak", "havacılık", "ilginç bilgiler",
                  "merak", "neden", "bilim", "teknoloji", "açıklama",
                  "biliyor muydunuz"],
    "karakter_sayfasi": "",
    "sahneler": [
        ...
        {"no": 3, "karakter_sahnede": False,
         "anlatim": "Beyaz, güneş ışığını en iyi yansıtan renktir. "
                    "Koyu bir gövde çok daha fazla ısınırdı.",
         "gorsel_prompt": "sunlight reflecting off a bright white curved surface, "
                          "strong highlights, dramatic contrast, macro shot"},
        {"no": 4, "karakter_sahnede": False,
         "anlatim": "Isınan bir uçağı soğutmak enerji ister. Bu da yakıt demek.",
         "gorsel_prompt": "aircraft engine turbine seen from below against a "
                          "clear sky, low angle shot"},
        {"no": 5, "karakter_sahnede": False,
         "anlatim": "Bir sebep daha var. Beyaz zeminde çatlak ya da yağ sızıntısı "
                    "hemen göze çarpar.",
         "gorsel_prompt": "a technician inspecting a white aircraft wing surface "
